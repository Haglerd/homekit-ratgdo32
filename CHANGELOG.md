...
### v3.4.4-forceclose.46 (2026-05-05)

Logs UX hotfix bundle. Three independent issues, all surfaced post-v44 deploy. v46 supersedes the open v45 PR (auth-skip alone) — that change is included here.

**SSE clientWrite skip noise rate-limited (the critical fix).** `web.cpp:362-381` (`clientWriteEx` flow-control skip path) now logs the `ESP_LOGD` once per 60 seconds instead of every occurrence. Verified against `/var/log/ratgdo.log` on the syslog Pi: 260 buffer-full skip lines per day (~7% of all log volume), concentrated in bursts when an SSE subscriber is wedged. That volume dominated the 16KB on-device log buffer and wrapped out short-lived user-action lines (force-close sequences, auto-close fires) before `/showlog` could capture them. The cumulative `sseBufferFullSkips` counter still increments on every skip; the log line just samples once per minute with a `[N total skips]` suffix so the rate is still visible. `ESP_LOGW` for slow writes (`CLIENT_SLOW_WRITE_MS` exceedance) and `FAILED` returns are unchanged.

**Force-close lines visible at INFO again.** `comms.cpp:2750/2766/2782` ("attempt N release sent", "scheduling attempt N+1", "attempt N/2 press hold=Xms") promoted from `ESP_LOGD` back to `ESP_LOGI`. v32's "MH2 sub-2 — force-close 6 INFO → 3" cleanup demoted these as "routine state", but force-close is a momentary user-triggered action, not routine state — visibility on each press attempt matters when verifying the hold-to-close override fired correctly. Verified via syslog comparison (May 4 vs May 5 — pre-v32 force-close traces had every press attempt at INFO; v46 restores that). The already-INFO `comms.cpp:2842` ("starting 2-attempt sequence") and `:2774` ("2-attempt sequence complete") + the WARN at `:3062` (auto-close fire) are unchanged.

**`/logs.html` re-prompt on every refresh fixed.** `logs.js:loadLogs()` now caches the timestamp of its last successful `/auth` call in `localStorage` (key: `ratgdo-logs-last-auth-at`) and skips the call entirely if it's been less than 4 minutes. The device's per-IP recent-auth allowlist (`web.cpp` `AUTH_ALLOWLIST_TTL_MS = 5 min`) is still warm in that window, so the SSE subscribe passes the `isAuthAllowedForIP()` check without re-stamping. Browsers that don't persist Digest credentials across navigations (Chrome's modern privacy default, Safari with aggressive cache-clearing, etc.) no longer re-prompt for the password every refresh. After the 4-min window the `/auth` call fires normally; on 401 the cached timestamp is cleared. (This was the v45 PR — folded into v46 per user direction.)

**Out of scope (still deferred from v44 + v45)**: W25 (`web_loop` rate limit, needs soak data), boot-time heap → `tiT` mDNS-OOM (architectural).
