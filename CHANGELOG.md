...
### v3.4.4-forceclose.45 (2026-05-05)

Logs UX hotfix — eliminates the re-prompt-on-every-refresh annoyance reported on v44. **`logs.js`** now caches the timestamp of its last successful `/auth` call in `localStorage` and skips the call entirely if it's been less than 4 minutes (the device's per-IP recent-auth allowlist has a 5-minute TTL, so the SSE subscribe will pass the `isAuthAllowedForIP()` gate without re-stamping). Symptom this fixes: browsers that don't persist Digest credentials across page navigations (Chrome's modern privacy-mode default, some Safari configurations, anything with aggressive cache-clearing) would re-prompt for the password every time the user refreshed `/logs.html` because v40's design called `/auth` on every page load. Pre-v45 the behavior was: refresh → fetch `/auth` → server returns 401 + Digest challenge → browser shows native auth dialog. Now: refresh → check localStorage → < 4 min ago, skip `/auth` → straight to SSE subscribe → no dialog. Allowlist expiry still re-prompts naturally after the 4-min skip window. Other auth flows unchanged: `/setgdo`, `/reboot`, `/reconnectHomeKit`, `/refreshHomeKitMDNS`, `/showlog`, `/showrebootlog`, `/crashlog` still go through the full `AUTHENTICATE()` macro every call (correct — those don't share the SSE allowlist semantics). No firmware-side change.
