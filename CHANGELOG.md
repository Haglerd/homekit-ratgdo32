...
- **Web-asset minification** (~21 KB flash). `build_web_content.py` now minifies JS/HTML/CSS (terser / html-minifier-terser / clean-css-cli) **before** the CRC computation and gzip, so the `?v=CRC` cache-busting matches the minified body. Source files are NOT changed — minification is build-time only, comments stay in source. Fail-safe: if a tool is missing or output is empty/implausibly small, it falls back to the raw bytes with a WARNING; terser output is gated by `node --check`. Gzipped savings: functions.js 24.7→13.8 KB, logs.js 6.2→2.2 KB, qrcode.js 9.8→7.0 KB, index.html 10.0→8.1 KB, plus the smaller assets. `.github/workflows/release.yml` gained `setup-node` + `npm ci` so CI-built release bins are minified and byte-identical to local builds (minifier versions pinned in `package.json`/`package-lock.json`).
- **Config string-getter torn-read race** (the W42 follow-up that was deferred and never filed). The string accessors (`getDeviceName`, `getLocalIP`, `getSyslogIP`, `getTimeZone`, `getwwwCredentials`, …) dereferenced the live config buffer pointer **outside** the mutex, so a concurrent `set()` (`strlcpy` under the lock) could tear the read. Fixed with a signature-preserving per-key snapshot copied under the same mutex (`getStr()`); **no call site changed**. ESP32-only (per-key slots, ~308 B); ESP8266 is single-threaded so it returns the live pointer with zero added cost. Latent in current code (all readers run on loopTask) — this is future-proofing for any later cross-task string reader.
- **Web NULL-deref guards** (`helperCredentials`, `helperUpdateUnderway`): the hand-rolled credential/firmware POST parsers did `strchr(x, ':') + 1` with no null check → crash on a malformed but authenticated POST. Each `strchr` result is now guarded; malformed input is rejected (`return false`) instead of dereferencing NULL.
- **Web-UI error-handling**: `checkVersion()` wraps its fetch in try/catch (an offline device no longer leaves the "checking…" spinner running forever); `firmwareUpdate()` checks `response.ok` before consuming the body (so the known OTA-Pages-lag 404 HTML is no longer MD5-hashed).
- **Vehicle off-debounce fix** (disco only): the clear condition was `off_counter / distanceInRange.size() >= PRESENCE_DETECTION_OFF_DEBOUNCE`, which required ~512 consecutive 0%-in-range samples (and `off_counter` resets on any percent change), so the "vehicle departed" clear effectively never fired through this path. Now `off_counter >= PRESENCE_DETECTION_OFF_DEBOUNCE` (2), matching the constant's documented intent. Verify on the sensor.
- **Files**: `build_web_content.py`, `package.json`, `package-lock.json`, `.github/workflows/release.yml`, `src/config.h`, `src/config.cpp`, `src/web.cpp`, `src/www/functions.js`, `src/vehicle.cpp`, `CHANGELOG.md`, `docs/manifest.json`.

### v3.4.4-forceclose.97 (2026-06-03) — Heap-floor + flash headroom reclamation

RAM and flash headroom pass. Behavior-preserving; FSM untouched; built clean on all 3 ESP32 envs. **Heap floor ~64 B → ~8 KB; flash 96.7% → 96.0%.**

- **Log ring buffer `LOG_BUFFER_SIZE` 16 KB → 8 KB (ESP32)** — the single biggest lever, and the only one that returns memory directly to the runtime free-heap (it's a `malloc`). On `.96` the runtime heap floor was dipping to ~64 B min-free-ever; this lifts it ~8 KB. Crash capture is unaffected (the RTC crash snapshot uses a separate `LOG_SAVE_BUFFER_SIZE` = 3584 B and stays well below the 8 KB ring); live syslog UDP forwarding is per-line at emit and independent of the ring. ESP8266 value untouched (it's in its own `#ifdef ESP8266` branch).
- **Status-JSON copy buffers merged** — the two `STATUS_JSON_BUFFER_SIZE` (2560 B) scratch copies `localJson` (web_loop SSE broadcast) and `status_json_send` (handle_status) became one shared `statusJsonCopy` (~2.5 KB .bss). Both are loopTask-only and strictly sequential within one `web_loop` iteration. The global malloc'd `status_json` scratch was deliberately NOT merged (it's read cross-task under the mutex; merging would reintroduce a torn-buffer race).
- **`SSE_MAX_CHANNELS` 8 → 4** (~0.7 KB .bss) — 4 concurrent browser/EventSource viewers (3 + reconnect slack). `MAX_CONCURRENT_REQUESTS` left at 8 (it's a counter, not a buffer).
- **Dropped `marked.umd.js`** (~12.5 KB flash) — replaced with a ~30-line escape-first `mdToHtml()` shim for rendering GitHub release notes in the update dialog (HTML-escapes before formatting; NUL-delimited code-block placeholders; balanced-paren link URLs).
- **Dropped `magic_enum`** (one `eTaskState` stringify → a `static const char*[6]` lookup) and removed the dead `src/www/qrcode.svg` (the live pairing QR is JS-generated via `qrcode.js` → `#qr-svg`).
- **Files**: `lib/ratgdo/log.h`, `src/web.cpp`, `src/utilities.cpp`, `src/utilities.h`, `platformio.ini`, `src/www/functions.js`, `src/www/index.html`, deleted `src/www/marked.umd.js` + `.map` + `src/www/qrcode.svg`, `CHANGELOG.md`, `docs/manifest.json`.
...
**1. New ESP32-only `GET /heap` endpoint (010)** returning free heap / `esp_get_minimum_free_heap_size` / largest free 8-bit block / `MALLOC_CAP_INTERNAL` free + largest / `MALLOC_CAP_SPIRAM` free / RSSI / uptime / active sampler cadence as JSON. Stack-local 256 B buffer — **no dynamic allocation in the handler**, since the whole point is that it must be queryable while the heap is under pressure. No auth, matching `/status.json` and `/crashlog` precedent.

**2. Adaptive HomeKit-health-sampler cadence (010)** in `homekit_health_log()`: drops from 180s to 30s when free heap < 20 KB, holds the fast cadence for 5 min after recovery (so we don't oscillate around the watermark). Watermark is intentionally above the `log.cpp` `SYSLOG_HEAP_FLOOR_BYTES=4096` — we start fast-sampling BEFORE the syslog-floor kicks in and we lose network fan-out for diag lines. Sampler re-arms `homekitHealthTicker` from inside the callback (safe; `esp_timer` documented to allow stop/start from within the timer's own callback).

**3. `tickDriftMs` field uses 64-bit time math.** Pre-fix at `src/homekit.cpp:688` double-cast `_millis()` (int64 on ESP32) through `uint32_t` then back to `int32_t`, producing garbage values once the absolute delta exceeded ~24.8 days. Realistic exposure was small (the variable is fresh-per-tick), but the cast was wrong regardless. New form `(int32_t)((int64_t)(_millis() - lastTickMs) - (int64_t)HOMEKIT_HEALTH_INTERVAL_MS)` eliminates the unsafe down-cast. Portable across ESP32 (int64 _millis_t) and ESP8266 (uint32 _millis_t wraps cleanly mod 2^32 then zero-extends).

**4. SEC1 TX retry log enriched with root-cause attribution (011).** The `SEC%d TX send failed, exceeded max retry` line now carries `[cmd=0xNN retries=N bus_silent=Nms cts=N rx_pending=N]` so we can distinguish bus-busy vs collision vs internal-RX-pending vs lost-ack as the underlying cause without having to correlate against adjacent RX traces. Both the first-of-burst and the `[+N suppressed]` summary line carry the new prefix. Format strings are static `.rodata` literals; stack frame grows ~16 B per call. Suppression cadence and delivery semantics unchanged.

**5. HomeKit Open/Close target-state dispatch dedup (008+009).** iOS HomeKit periodically fires bursts of 4-13 redundant identical-target characteristic writes within 1-2 s for state-sync verification. Pre-dedup every burst write reached `open_door() / close_door() / door_command_force_close()` — producing SEC1 packet cascades (008 Open path) and (009 Close path) a real risk of a second force-close sequence firing inside the cross-task state-propagation race. New `hk_target_is_redundant()` helper drops requests whose target matches `garage_door.current_state` OR is already-transitioning-toward-target (CURR_OPEN+OPEN, CURR_OPENING+OPEN, CURR_CLOSED+CLOSED, CURR_CLOSING+CLOSED). **Mid-cycle reversals preserved** (CURR_OPENING+CLOSED and CURR_CLOSING+OPEN fire normally — the GDO wall-button toggle honors a reversal and eating it would leave the door doing the opposite of what the user just asked for). Helper returns true so HomeKit sees the characteristic-write ack and the iOS retry loop settles. Rate-limited summary log uses the same 5s-window / `[+N suppressed]` shape as the existing comms.cpp rate-limits; separate Open/Close counters so direction is observable in `/showlog`. Wired into both `DEV_GarageDoor::update()` (tag `"primary tile"`) and `DEV_GarageDoorForceClose::update()` (tag `"FC tile"`).

**6. Force-close reentry cooldown (009 second-FC fix).** Strictly additive to the existing CURR_CLOSED/CLOSING/0xFF safety gate in `door_command_force_close`. `clear_force_close_state` now stamps a release-store `forceCloseClearedAtMs` BEFORE the existing `__atomic_clear(&forceCloseInProgress)`; entry into `door_command_force_close` does an acquire-load and refuses for 3 s if the previous sequence just cleared. Closes the cross-task race where the comms task clears `forceCloseInProgress` but the HomeSpan task hasn't yet seen `garage_door.current_state` propagate to CURR_CLOSING (audit observed 1.34 s after first press cleared). Safety: first-ever press passes (ClearedAtMs == 0), legitimate retry minutes later passes (elapsed >> 3000), auto-close TTC fires minutes-scale apart so passes, ESP8266 uint32 wrap between store/load produces a huge unsigned delta = passes (fail-OPEN). **Does NOT modify the .80 release-callback fix, the 2-attempt mechanic, or the existing safety gate.**

**ESP8266 build unchanged for the 010 visibility additions.** Every new 010 line gated under `#ifndef ESP8266`. The tickDriftMs / SEC1 enrichment / HK dedup / FC cooldown changes are portable (no `#ifdef` needed; logic uses portable types and atomics, no buffer growth). ESP8266 builds today only via the placeholder env — no env exists in `platformio.ini` for an ESP8266 target, so build verification is ESP32-only.

**Files**: `src/web.cpp` (new endpoint), `src/homekit.cpp` (adaptive cadence + tickDriftMs + HK dedup), `src/comms.cpp` (SEC1 enrichment + FC reentry cooldown), `CHANGELOG.md`, `QUEUE.md`. No `manifest.json` bump in this commit — paired with the release-bump PR step after merge.
