...
10a. **On code-review out-of-scope-but-real findings**: when code-review surfaces a real issue that is pre-existing / not a regression introduced by this PR / out-of-scope for the current item, **DO NOT silently drop it**. Classify each review finding into one of: (a) in-scope regression → fix in this PR; (b) architectural → step 10 planner-revision; (c) **out-of-scope-but-real → file as a NEW queue item in QUEUE.md** with brief description, source `code-review on PR #<n>`, suggested priority based on severity, complexity guess. List the deferred queue items in the PR description so the user sees them on review. Commit the new queue items in the same branch as the PR. **Mandatory** — "pre-existing, not a regression" is not a halt reason and not a drop reason; it's a queue-and-continue reason.
