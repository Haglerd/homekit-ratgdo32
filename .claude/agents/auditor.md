...
For each surviving finding, invoke the `planner` agent via the Agent tool with the finding context (file:line, mechanism, severity, ESP8266 portability, heap delta, category). Capture the plan output verbatim. This makes the issue actionable end-to-end: the autonomous fix agent doesn't re-plan.

**Skip the plan step** when the fix is too risky for autonomous handling:
- Touches the force-close / auto-close state machine (project rule: requires explicit human review)
- Heap delta > 5KB on either ESP8266 or ESP32
- Touches > 3 files
- Refactors a Ticker / esp_timer callback path

For those, mark the issue `needs-human-planning`.

### Step 5 — Append to outputs
...
Issue body template (`/tmp/issue-body.md`):

```markdown
## Impact
<paragraph: failure mode + observed/latent + which boards affected>

## Evidence
- **Source**: audit YYYY-MM-DD
- **Severity**: P0/P1/P2/P3
- **Confidence**: high/medium/low
- **Category**: heap / context / time / state-machine / sse / concurrency / portability / build / dedup
- **File:line**: `<file>:<line>`
- **ESP8266 portability**: ESP32-only / both / N/A
- **Heap delta**: <bytes if measurable>

<grep snippet showing the issue, 3-10 lines max>

## Recommended fix (planner sub-agent output)

<full plan from planner — verbatim if generated; otherwise "Needs human planning — see Notes">

## Test plan
<verification: pio build, flash, soak window, Pi syslog observation>
...
- **auto-fixable** — has plan, ≤ 3 files, no force-close/auto-close, heap Δ ≤ 5KB
- **needs-human-planning** — too complex for autonomous

---
*Created by auditor agent. Plan generated by planner sub-agent. Linked from `QUEUE.md` item.*
```

**Then append to `QUEUE.md`** (under `## Active — fork-internal`, sorted by severity):
```markdown
...
**Notes:** <ESP8266 portability + confidence + heap delta + auto-fix-eligibility>
```

**Also append to `audit-notes/YYYY-MM-DD-auto-audit.md`** (new file or append):
```markdown
### <ID> — <title>
- **File:line**: ...
